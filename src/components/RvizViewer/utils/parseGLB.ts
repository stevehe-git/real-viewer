/**
 * GLB 文件解析工具函数
 * 完全基于 regl-worldview 的 parseGLB.js 实现
 * 
 * 注意：如果 GLB 文件使用 Draco 压缩，需要安装 draco3d 依赖
 * npm install draco3d
 */
...
export type GLBModel = {
  json: any
  accessors?: (TypedArray | null)[]
  images?: ImageBitmap[]
}

/**
 * 解析 GLB 文件: https://github.com/KhronosGroup/glTF/tree/master/specification/2.0
 * 
 * 返回包含原始 JSON 数据以及解析后的图像 (Image) 和访问器 (TypedArray) 的对象
...
export default async function parseGLB(arrayBuffer: ArrayBuffer): Promise<GLBModel> {
  ...
  function findNextChunkOfType(type: number): DataView | undefined {
    do {
  ...
  const accessors = json.accessors.map((accessorInfo: any) => {
    ...
    (await Promise.all(
      json.images.map((imgInfo: any) => {
        ...
        return globalThis.createImageBitmap(
          new Blob([imageData], { type: imgInfo.mimeType })
        )
      })
    ))
