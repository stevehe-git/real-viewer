...
const mapTextureCommand = (regl: Regl) => {
  ...
  return {
    ...
    // 参照 RViz 的实现，优化多个地图叠加时的深度处理策略
    // 1. 启用深度测试，确保地图能正确与网格和其他地图交互
    // 2. 禁用深度写入（mask: false），避免多个地图之间的 Z-fighting
    //    因为每个地图已经有唯一的 Z 偏移，不需要深度写入
    // 3. 使用 'less' 进行深度测试，只渲染更近的像素（配合 Z 偏移使用）
    // 4. 网格在 Z=0.0001，地图在 Z>=0（正常）或 Z<0（drawBehind）
    // 5. 多个地图之间通过 Z 偏移（0.001 间隔）确保正确的深度排序
    depth: {
      enable: true,
      mask: false, // 禁用深度写入，避免多个地图之间的 Z-fighting
                   // 每个地图已经有唯一的 Z 偏移，不需要深度写入
      func: 'less' // 使用 'less' 只渲染更近的像素，配合 Z 偏移确保正确的叠加
    },
    blend: defaultBlend,
    count: 6 // 2个三角形 = 6个顶点
  }
