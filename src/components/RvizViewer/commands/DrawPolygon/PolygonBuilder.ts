/**
 * PolygonBuilder 类
 * 完全基于 regl-worldview 的 DrawPolygon/PolygonBuilder.js 实现
 * 用于构建和编辑多边形
 */
// @ts-ignore - distance-to-line-segment 可能没有类型定义
...
function areEqual(point1: PolygonPoint, point2: PolygonPoint): boolean {
...
// based on mouse & keyboard interactions. For now we use mutation internally
...
export default class PolygonBuilder {
  ...
  polygons: Polygon[]
  onChange: () => void = () => {}
  activePolygon: Polygon | null = null
  activePoint: PolygonPoint | null = null
  mouseDownPoint: Vec3 | null = null
  ...
  onMouseMove: MouseHandler = (e, args) => {
    ...
    if (this.activePoint) {
      ...
      const uniquePoints = activePolygon.points.reduce((acc: PolygonPoint[], point) => {
  ...
  selectObject(object?: Polygon | PolygonPoint): void {
